...
<script>
    $(document).ready(function() {
        ...
        // Auto-trigger load on Edit view start
        if ($('#branch_id').val()) {
            loadBranchEmployees();
        }
        ...
        $(document).on('submit', '#attendance-save-form', function(e) {
            ...
            let missingLeaveNotDeducted = [];
            $('.employee-row').each(function() {
                let row = $(this);
                let empName = row.attr('data-emp-name');
                ...
                if (leaveTakenVal !== '' && leaveNotDeductedVal === '') {
                    leaveNotDeductedInput.addClass('is-invalid');
                    missingLeaveNotDeducted.push(empName);
                }
            });

            if (missingLeaveNotDeducted.length > 0) {
                let errorMsg = 'Please enter <strong>Leave Not Deducted</strong> for the following employee(s): <strong>' + missingLeaveNotDeducted.join(', ') + '</strong> (Leave Taken is entered).';
                $('#alert-container').html('<div class="alert alert-danger alert-dismissible fade show" role="alert"><i class="bi bi-exclamation-triangle-fill me-2"></i>' + errorMsg + '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                $('html, body').animate({ scrollTop: $('#alert-container').offset().top - 70 }, 300);
                return false;
            }
